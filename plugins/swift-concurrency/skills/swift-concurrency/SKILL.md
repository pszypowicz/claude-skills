...
- **@Observable view models**: isolate the entire class to `@MainActor` when it drives UI; use `nonisolated` for computed properties or methods that don't touch UI state.
...
8. When using `@Observable`, verify that UI-driving properties are accessed on `@MainActor`.
